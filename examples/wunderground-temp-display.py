...
#
# Python script to grab Wunderground temperature data and display on matrix11x7. This script shows the current temperature along with an indicator of
...
#
# Development environment: Python v2.7.13 on a Raspberry Pi Zero-W running Raspbian "stretch" and default matrix11x7 libraries
#
...
#
# Also note that if you receive an odd "Remote I/O" error, that's the matrix11x7's odd way of saying that it can't
...
def draw_kr_pulse(pos, dir):
    ...
    matrix11x7.clear_rect(12, 5, 5, 1)
    x = pos + 11                       # increase position to the actual x offset we need
    matrix11x7.set_pixel(x, 5, 0.2)  # turn on the current pixel
    matrix11x7.show()
...
def draw_temp_trend(dir):
    ...
    if dir == 0:                                   # equal - don't display anything. Clear the area where direction arrow is shown
        matrix11x7.clear_rect(14, 0, 3, 6)
    elif dir == 1:                                 # increasing = up arrow. Draw an up arrow symbol on the right side of the display
        for y in range(0, 5):
            matrix11x7.set_pixel(15, y, BRIGHT)  # draw middle line of arrow
        matrix11x7.set_pixel(14, 1, BRIGHT)      # draw the 'wings' of the arrow
        matrix11x7.set_pixel(16, 1, BRIGHT)
    elif dir == -1:                                # decreasing = down arrow
        for y in range(0, 5):
            matrix11x7.set_pixel(15, y, BRIGHT)  # draw middle line of arrow
        matrix11x7.set_pixel(14, 3, BRIGHT)
        matrix11x7.set_pixel(16, 3, BRIGHT)
...
def draw_wind_line():
    ...
    for x in range(0, wind_calc):
        matrix11x7.set_pixel(x, 6, WIND_BRIGHTNESS)
    # Now draw the gust indicator as a single pixel
    if gust_calc:  # only draw if non zero
        matrix11x7.set_pixel(gust_calc - 1, 6, GUST_BRIGHTNESS)
    return
...
def display_temp_value():
    ...
    if current_temp < 100:
        matrix11x7.clear_rect(0, 0, 12, 5)
    else:
        matrix11x7.clear_rect(0, 0, 17, 5)
    if CURRENT_TEMP_DISPLAY == 1:  # show actual temp
        matrix11x7.write_string(actual_str, x=0, y=0, font=font3x5, brightness=BRIGHT)
    else:                          # show feels_like temp
        matrix11x7.write_string(feels_like_str, x=0, y=0, font=font3x5, brightness=BRIGHT)
    matrix11x7.show()
    time.sleep(1)
    return
...
print("Uses Raspberry Pi-W and 11x7 Matrix display. Written by Mark Ehr, January 2018")
